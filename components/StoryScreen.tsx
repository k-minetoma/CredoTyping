  ...
  return (
    ...
    >
      <div className="absolute top-0 left-0 w-full h-full flex justify-center items-center p-16 pointer-events-none">
        {/* New Employee Character（左・少し小さめ） */}
        <div
          className={`transition-all duration-500 ease-out transform ${
            isPresidentSpeaking ? 'opacity-50 scale-90' : 'opacity-100 scale-90'
          }`}
          style={{ marginLeft: '-100px' }}
        >
          <img
            src={CHARACTER_DATA['あなた'].image}
            alt={CHARACTER_DATA['あなた'].name}
            className="h-[70vh] max-h-[500px] object-contain drop-shadow-lg"
          />
        </div>

        {/* President Character（右・標準サイズ） */}
        <div
          className={`transition-all duration-500 ease-out transform ${
            isPresidentSpeaking ? 'opacity-100 scale-100' : 'opacity-50 scale-95'
          }`}
          style={{ marginRight: '-100px' }}
        >
          <img
            src={CHARACTER_DATA['社長'].image}
            alt={CHARACTER_DATA['社長'].name}
            className="h-[80vh] max-h-[600px] object-contain drop-shadow-lg"
          />
        </div>
      </div>
